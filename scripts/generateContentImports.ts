...
const generateContentLoaderModule = (markdownFilePaths: string[]): string => {
  ...
  for (const relativeFilePath of markdownFilePaths) {
    try {
      ...
      const blogPost = {
        ...
        topic: topicFromPath,
        content: '', // Content will be loaded lazily at runtime
  ...
  const generatedModuleContent = `// Auto-generated content metadata - do not edit manually
...
const executeContentGeneration = (): void => {
  ...
  const outputFilePath = path.join(
    SCRIPT_DIRECTORY,
    '../src/lib/content/generatedMetadata.ts'
  )
